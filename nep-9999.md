...
One major difference is having `policy.rs` in the Sputnik DAO code.
...
Vostok proposals are a good example of a simpler model that we are considering:
...
* `get_delegated_amount(delegator_id: AccountId) -> PromiseOrValue<Balance>`: returns the amount of tokens delegated by the given account.
...
An implementation of the `Action` enum is also defined with a method `to_policy_label()` that returns a string representation of the variant.
